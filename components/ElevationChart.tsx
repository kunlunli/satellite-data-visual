...
export default function ElevationChart({ data, currentIndex, height = 200 }: Props) {
  ...
  return (
    <div className="relative bg-white rounded-lg p-3 shadow-sm">
      <h2 className="text-xs font-semibold text-gray-600 text-center mb-2">Elevation (deg)</h2>
      <button
        type="button"
      ...
      <ResponsiveContainer width="100%" height={height}>
        <LineChart data={chartData} margin={{ top: 4, right: 16, bottom: 24, left: 32 }}>
          ...
          <Line type="monotone" dataKey="cur" name="Current EL" stroke="#3b82f6" dot={showDots ? { r: 4, fill: '#3b82f6', strokeWidth: 0 } : false} strokeWidth={1.5} isAnimationActive={false} />
          <Line type="monotone" dataKey="target" name="Target EL" stroke="#f97316" dot={showDots ? { r: 4, fill: '#f97316', strokeWidth: 0 } : false} strokeWidth={1.5} isAnimationActive={false} />
